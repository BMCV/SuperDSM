...
This also concerns BLAS, which is pinned to ``blas==1.0``. Analogously to the Conda environment, *requirements.txt* specifies the required *pip* dependencies with pinned versions. However, to the best of our knowledge, it is not possible to request a specific BLAS version in *pip*, meaning that using *pip* instead of Conda is discouraged.
...
Both specifications are included in the Conda environment specified in *superdsm.yml*.
