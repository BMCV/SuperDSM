...
def require_data(data_id):
    data_path = data_dir / data_id
    if not data_path.exists():
        with open(data_dir / 'database.json') as dbfp:
            db = json.load(dbfp)
        data = db[data_id]
        load_data(data['url'], data['datasets'])
    return data_path
...
def validate_image(test, name, img):
    ...
    try:
        test.assertTrue(np.allclose(img, expected)) ## TODO: write "wrong" result to file if check fails
    except:
